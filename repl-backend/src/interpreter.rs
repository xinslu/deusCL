...
impl Interpreter {
    ...
    pub fn accept(&mut self, expression: Expression) -> Result<(), Error> {
        match expression {
            ...
            _ => Err(Error::Reason(format!(
                "{:?} Unsupported Operation Right Now",
                expression
            ))),
        }
    }
    ...
    pub fn process(&mut self, expression: Expression) -> Result<Option<Values>, Error> {
        match expression {
            ...
            Expression::Print { print: _ } => Ok(Some(self.visit_print(expression)?)),
            Expression::Variable { name: _ } => Ok(Some(self.visit_literal(&expression)?)),
            Expression::Literal { ref token } => {
                if let TokenTypes::STRINGLITERAL = token._type {
            ...
            Expression::Call { .. } => Ok(Some(self.visit_call(expression)?)),
            Expression::Global { .. } => {
                self.visit_global(expression)?;
                Ok(None)
            }
    ...
    pub fn comparision_lambda(
        &mut self,
        expr: Vec<Expression>,
        func: &dyn Fn(i64, i64) -> bool,
    ) -> Result<bool, Error> {
        if expr.len() as i32 == 0 {
            return Err(Error::Reason(
                "Parsing Error, not enough arguements".to_string(),
            ));
        }
    ...
    pub fn artihmetic_lambda(
        &self,
        expr: Vec<Expression>,
        func: &dyn Fn(i64, i64) -> i64,
    ) -> Result<i64, Error> {
        let mut temp = (self.visit_literal(&expr[0])?).matchInteger()?;
        for i in &expr[1..] {
    ...
    pub fn string_lambda(
        &self,
        expr: Vec<Expression>,
        func: &dyn Fn(String, String) -> String,
    ) -> Result<String, Error> {
...
impl Visitor for Interpreter {
    fn visit_logical(&mut self, log: Expression) -> Result<bool, Error> {
        ...
        let or = |a, b| a || b;
        match log {
            Expression::Logical { operator, expr } => match operator._type {
                TokenTypes::EQUAL => {
                ...
                TokenTypes::NOT => {
                    if expr.len() > 1 {
                        return Err(Error::Reason(
                            "Cannot Have more than 1 Arguement".to_string(),
                        ));
                    }
                    match &expr[0] {
                        Expression::Literal { token } => {
                ...
                _ => Err(Error::Reason("Unsupported Operator".to_string())),
            },
            _ => Err(Error::Reason("Not a Logical Expression".to_string())),
        }
    }
    ...
    fn visit_literal(&self, lit: &Expression) -> Result<Values, Error> {
        match &lit {
            ...
            _ => Err(Error::Reason("Not a Literal!".to_string())),
        }
    }
    ...
    fn visit_arithmetic(&self, arith: Expression) -> Result<i64, Error> {
        ...
        let modulus = |a, b| a % b;
        match arith {
            Expression::Arithmetic { operator, expr } => match operator._type {
                TokenTypes::MAX => {
                ...
                _ => Err(Error::Reason("Not a Valid Type!".to_string())),
            },
            _ => Err(Error::Reason("Not an Arithmetic Expression".to_string())),
        }
    }
            ...
            for i in declarations {
                if let Expression::Assignment { name, expr } = i {
                    if let Expression::Variable { name } = *name {
                        ...
                        return Err(Error::Reason(
                            "Not a Variable in declared local block".to_string(),
                        ));
                    }
                } else {
    ...
    fn visit_set(&mut self, set: Expression) -> Result<(), Error> {
        match set {
            ...
            _ => Err(Error::Reason("Invalid Operation".to_string())),
        }
    }

    fn visit_print(&mut self, print: Expression) -> Result<Values, Error> {
        match print {
            ...
            _ => Err(Error::Reason("Illegal Operation".to_string())),
        }
    }
    ...
    fn visit_if(&mut self, ifBlock: Expression) -> Result<(), Error> {
        match ifBlock {
            Expression::If {
                ...
                _ => Err(Error::Reason("Not a logical condition".to_string())),
            },
            _ => Err(Error::Reason("Invalid Type of Operator".to_string())),
        }
    }
    ...
    fn visit_string(&mut self, string: Expression) -> Result<String, Error> {
        match string {
            ...
            _ => Err(Error::Reason("Not a String".to_string())),
        }
    }
    ...
    fn visit_global(&mut self, global: Expression) -> Result<(), Error> {
        if let Expression::Global { name, expr } = global {
            ...
            Err(Error::Reason(
                "Not a Global Variable Declaration".to_string(),
            ))
        }
    }
    ...
    fn visit_return(&mut self, ret: Expression) -> Result<(), Error> {
        if let Expression::Return { result } = ret {
            match self.process(*result)? {
                Some(x) => return Err(Error::Return(x)),
                None => return Err(Error::Reason("Return cannot be empty".to_string())),
            }
        } else {
    ...
    fn visit_function_dec(&mut self, func: Expression) -> Result<(), Error> {
        if let Expression::Function { ref name, .. } = func {
            self.environment
                .define(name.to_string(), Values::Function(func));
            Ok(())
        } else {
    ...
    fn visit_call(&mut self, call: Expression) -> Result<Values, Error> {
        if let Expression::Call { name, parameters } = call {
            ...
            if parameters.len() as i64 == function.arity()? {
                    ...
                    match self.process(i.clone())? {
                        ...
                        None => {
                            return Err(Error::Reason(format!(
                                "Function Argument {:?} does not evaluate to a valid value",
                                i
                            )))
                        }
                    }
                ...
                return Err(Error::Reason(format!(
                    "Invalid Call to function, expected {} arguements got {}",
                    function.arity()?,
                    parameters.len()
                )));
            }
